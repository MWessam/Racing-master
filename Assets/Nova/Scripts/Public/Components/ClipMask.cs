...
namespace Nova
{
    ...
    public sealed class ClipMask : MonoBehaviour
    {
        ...
        [SerializeField] private Sprite maskSprite = null;

        internal void RegisterOrUpdate()
        {
            if (maskSprite != null && maskTexture != null)
            {
                Rect spriteRect = maskSprite.textureRect;
                ...
                Color[] specificPixels = maskSprite.texture.GetPixels(x, y, width, height);
                maskTexture = new Texture2D(width, height);
                maskTexture.SetPixels(specificPixels);
            }
