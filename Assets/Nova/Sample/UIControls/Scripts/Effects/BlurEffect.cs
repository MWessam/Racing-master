...
namespace NovaSamples.Effects
{
    ...
    public class BlurEffect : MonoBehaviour
    {
        ...
        static private readonly Vector2 Vector2_Half = new Vector2(0.5f, 0.5f);
        ...
        [NonSerialized]
        private readonly TextureBlur textureBlur = new TextureBlur();
        ...
        static private Rect GetViewportRect(UIBlock2D uiBlock, Camera camera)
        {
            if (uiBlock == null)
            {
        ...
        static private Rect GetViewportRect(Vector2 uiBlockSize, Matrix4x4 modelMatrix, Camera camera)
        {
        ...
        static private Vector2 ProjectedPointToViewport(Vector4 projected) => projected.w == 0 ? Vector2.zero : new Vector2((0.5f * projected.x / projected.w) + 0.5f, (0.5f * projected.y / projected.w) + 0.5f);
        static private Rect CreateRect(Vector2 center, Vector2 size) => new Rect(center - 0.5f * size, size);
        static private float AngleBetweenAroundAxis(Vector3 from, Vector3 to, Vector3 axis)
        {
