...
namespace NovaSamples.Effects
{
    ...
    internal class TextureBlur : IDisposable
    {
        ...
        static private readonly Vector4 DefaultColors = new Vector4(0, 1, 1, 1);

        [NonSerialized]
        static private uint NextInstanceID = 0;

        [NonSerialized]
        static private readonly int BlurID = Shader.PropertyToID("_BlurBox");
        [NonSerialized]
        static private readonly int UVScaleID = Shader.PropertyToID("_UVScale");
        [NonSerialized]
        static private readonly int InputTextureID = Shader.PropertyToID("_Input");
        [NonSerialized]
        static private readonly int OutputTextureID = Shader.PropertyToID("_Output");
        [NonSerialized]
        static private readonly int ColorAdjustmentID = Shader.PropertyToID("_ColorAdjustments");
        [NonSerialized]
        static private readonly int GrainID = Shader.PropertyToID("_Grain");
        [NonSerialized]
        static private readonly int OffsetID = Shader.PropertyToID("_Offset");
        [NonSerialized]
        static private readonly int ClipUVsID = Shader.PropertyToID("_ClipUVs");
        [NonSerialized]
        static private readonly int GrainUVsID = Shader.PropertyToID("_GrainUVs");
        #endregion
        ...
        private readonly Material[] blurMaterials = new Material[MaxPasses];

        [NonSerialized]
        static private Shader blurShader = null;
        static private Shader BlurShader
        {
            get
            {
                if (blurShader == null)
        ...
        [NonSerialized]
        static private ComputeShader sharedComputeShader = null;
        static private ComputeShader SharedComputeShader
        {
            get
            {
        ...
        private readonly List<Vector4> computeBlurs = new List<Vector4>();
        private readonly int[] tempTextureIDs = new int[]
        {
        ...
        static private void Swap01<T>(T[] textures)
        {
            T temp = textures[0];
        ...
        static private Vector4 GetShaderUVScale(Texture image) => GetShaderUVScale(image.width, image.height);
        static private Vector4 GetShaderUVScale(int width, int height) => new Vector4(1f / width, 1f / height, width, height);

        static private Rect GetScissorRect(Vector2 blurableSize, Rect viewportRect, Vector2 blurUVs)
        {
        ...
        static private void Destroy<T>(T obj) where T : UnityEngine.Object
        {
            if (obj == null)
            {
                return;
        ...
        static private bool IsDefaultColors(Vector4 csb)
        {
        ...
        static private bool ComputeShaderSupported => SystemInfo.supportsComputeShaders && !IsMobileOpenGLLinear;
        ...
        /// </summary>
        static private bool IsMobileOpenGLLinear => QualitySettings.activeColorSpace == ColorSpace.Linear && IsOpenGL(SystemInfo.graphicsDeviceType) && Application.isMobilePlatform;
        static private bool IsOpenGL(GraphicsDeviceType deviceType) => deviceType == GraphicsDeviceType.OpenGLES3 || deviceType == GraphicsDeviceType.OpenGLCore;
    }
