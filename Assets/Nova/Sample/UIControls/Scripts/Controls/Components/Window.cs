...
namespace NovaSamples.UIControls
{
    ...
    public class Window : MonoBehaviour
    {
        ...
        /// </summary>
        private readonly Dictionary<uint, Bounds> cachedBounds = new Dictionary<uint, Bounds>();
        ...
        static private Vector3 Sign(Vector3 v) => new Vector3(Sign(v.x), Sign(v.y), Sign(v.z));
        ...
        static private float Sign(float f) => f < 0 ? -1 : f > 0 ? 1 : 0;
        ...
        /// </remarks>
        static private Vector3 ClampPositionDelta(Vector3 positionDelta, Vector3 sizeDelta, Vector3 currentSize, MinMax3 sizeRange)
        {
        ...
        /// Handles the case where if the UIBlock has a minimum (or maximum) size, we don't want to continue moving the UIBlock once that minimum (or maximum) size is reached while the user is making the window smaller (or larger).
        /// </remarks>
        static private float ClampPositionDelta(float rawPositionDelta, float sizeDelta, float currentSize, MinMax range)
        {
        ...
        static private void SetCursor(Texture2D cursorTexture)
        {
