...
public class RCCP_AxlesEditor : Editor {
    ...
    public override void OnInspectorGUI() {
        ...
        if (GUILayout.Button("Assign Wheels"))
        {
            var axles = prop.Axles;
            foreach (var axle in axles)
            {
                foreach (Transform child in axle.transform)
                {
                    if (child.name.Contains("Wheel_") && !child.name.Contains("Collider"))
                    {
                        if (child.name.Contains("_L"))
                        {
                            axle.leftWheelModel = child;
                        }
                        else if (child.name.Contains("_R"))
                        {
                            axle.rightWheelModel = child;
                        }
                    }
                }
            }
