...
public class RCCP_CarController : MonoBehaviour {
    ...
    public float OverridenMaxSpeed = 0.0f;
    #region COMPONENTS
    ...
    private void Drivetrain() {
        ...
        var speedCalculation = transform.InverseTransformDirection(Rigid.velocity).z * 3.6f;
        speed = speedCalculation >= maximumSpeed ? maximumSpeed : speedCalculation;
        ...
        if (!overrideMaxSpeed)
        {
            CalculateMaxSpeed(averagePowerWheelRadius);
        }
        else
        {
            if (NitroUsed)
            {
                maximumSpeed = OverridenMaxSpeed + NitroAmount;
            }
            else
            {
                maximumSpeed = OverridenMaxSpeed;
            }
        }
        //  Produced torques.
        if (Engine)
    ...
    public void CalculateMaxEngineRPMForMaxSpeed(float averagePowerWheelRadius, float maxSpeed)
    {
        var temp = (maxSpeed * lastGearRatio * differentialRatio * 10000) / (60 * 2 * Mathf.PI * averagePowerWheelRadius);
        maxEngineRPM = Engine.maxEngineRPM;
    }
    private void CalculateMaxSpeed(float averagePowerWheelRadius)
    {
        /*
         * maxSpeed = (maxEngineRPM / lastGearRatio) / differentialRatio
         * 
         */
